...
export function encrypt(data: {repoUrl: string, commitHash: string}, userAddress: string): string {
  const key = deriveKey(userAddress);
  const iv = crypto.randomBytes(12); // 96-bit IV for GCM
  const cipher = crypto.createCipheriv(algorithm, key, iv);
...
export function decrypt(encryptedData: string, userAddress: string): { repoUrl: string; commitHash: string } {
  const key = deriveKey(userAddress);
  const iv = Buffer.from(encryptedData.slice(0, 24), "hex");
  const tag = Buffer.from(encryptedData.slice(24, 56), "hex");
  const encrypted = encryptedData.slice(56);
